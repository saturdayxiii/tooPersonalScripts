...
$ytdlp = "C:\Users\Saturday\scoop\apps\yt-dlp\current\yt-dlp.exe"
...
& $ytdlp --no-overwrites --continue $playlistUrl -f bestvideo+bestaudio --output "$outputDirectory\$format" --write-subs --sub-lang en --embed-subs --add-metadata --metadata-from-title "%(url)s"
